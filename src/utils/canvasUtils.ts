...
export default async function getCroppedImg(
    ...
    const data = ctx.getImageData(
        ...
        pixelCrop ? pixelCrop.width : canvas.width,
        pixelCrop ? pixelCrop.height : canvas.height
    )
    ...
    return new Promise((resolve, reject) => {
        canvas.toBlob((file) => {
            if (file) {
                // Convert Blob to File, keeping original mime type helps, but default is fine
                // We'll create a generic name or pass the original name if needed
